...
调用前读取 [`../auth.md`](../auth.md)，验证 Bearer token、租户和当前用户。日报请求的 `Referer` 固定为本页；列表查询固定传 `type=1`。周报总结只读取日报，不写入系统；无明确变更确认时仅可调用读取接口。
...
- `dailyType` 表示填写人的固定日志类别，不是工作内容分类器。新增或补充日志时，读取当前用户最近一条 `dailyType` 有效的历史日志，所有待新增记录统一沿用该值；即使工作内容属于其他岗位或事项，也不得根据内容、关键词、客户名称或任务性质改猜类型。
- 用户明确指定 `dailyType` 时可按用户指定值处理；没有有效历史值且用户未指定时，展示允许值并询问，不能自行选择。修改已有日志时默认保留原记录的 `dailyType`，除非用户明确要求修改。
...
2. 构造查询；不传空筛选字段。周或日期查询直接传 `startDate`、`endDate`，不读取全量后在本地过滤。仅为判断 `dailyType` 或参考文风时，固定查询当前用户最近 20 条（`pageNum=1`、`pageSize=20`）并复用同一批结果，不继续翻页；计算补充日志的历史基线时，查询目标日期前连续 35 个自然日并遍历全部分页。周报要求的本周记录、目标日期已有记录等明确要求完整范围的查询也必须遍历该范围全部分页。
3. 检查 HTTP 状态和业务 `code`，返回 `data`、`count`、筛选条件和必要字段。

## 日报与周报文本风格

生成或润色 `workContent`、`completionStatus`、`tomorrow` 以及周报前，最多读取当前用户最近 20 条日志，并根据对话内容提取用户的表达习惯，例如常用称呼、句子长短、分项方式、标点和业务术语。已有查询结果足够时直接复用，不为参考风格重复请求；不能把历史中的旧客户、旧任务、结果或工时带入新内容。

- 事实以用户输入和本次查询到的日志为准；风格参考不能覆盖事实，也不能补写未发生的沟通、成果、进度或承诺。
- 表达自然、直接、偏口语，像本人整理工作记录；保留必要的产品名、项目名和技术词，不刻意换成正式公文表达。
- 一项只说一件主要事情，写清对象、做了什么、当前结果或下一步；合并同义重复，删除“赋能”“抓手”“深度聚焦”“全面提升”等空泛总结腔。
- 少用整齐对仗、机械的“首先/其次/最后”、重复句式和夸张评价。除非历史日志确实长期如此表达，否则不用“持续推进”“形成闭环”“有序开展”等模板化措辞。
- 用户给出的原文已经自然清楚时只做必要整理，不为显得完整而扩写。样本不足或风格不稳定时，采用简短、朴素、具体的默认写法，并说明未找到稳定的历史表达习惯。

## 生成周报总结

周报总结是只读文本生成，不调用新增或修改接口，不需要变更确认。

1. 以当前系统本地日期确定本周范围：周一至周日。查询条件使用本周周一的 `startDate`、本周周日的 `endDate` 和认证响应中的当前填写人；遍历全部分页，不能只总结第一页，也不能混入同部门其他人的日志。
2. 检查 HTTP 状态和业务 `code`。没有日志时明确说明本周暂无可总结的日报，不凭空生成周报。
3. 按业务对象或任务归并 `workContent` 和 `completionStatus`，把同一事项多天的重复描述合成一条；保留实际动作、问题、结果和未完成状态，不逐日机械抄写，也不把 `createTime` 当作业务发生日期。
4. 下周计划只取自本周日志的 `tomorrow`、明确未完成事项和用户在当前对话中补充的安排。可以把明确的后续动作整理得更顺，但不能新增任务、完成时间、负责人、客户承诺或预期结果。
5. 按“日报与周报文本风格”整理措辞，并在一个可直接复制的 Markdown 代码块中输出。代码块外只说明查询范围、日志条数和缺失事实；代码块内不加解释、免责声明或数据来源。
...
两部分都使用从 `1.` 开始的连续序号。每条围绕一个事项，避免多层嵌套；没有可靠的下周计划依据时，在“下周计划”下写 `1. 本周日志里暂时没记录明确的下周安排。`，不要猜测。

## 新增日志

1. 确定目标业务日期，调用 `weekByDay` 读取模板。
2. 仅编辑目标日期、当前用户的非空行；保留模板的身份和日期字段，移除没有 `id` 的纯空行。按“记录字段与校验”的 `dailyType` 规则读取历史类型，并统一写入本批全部记录，不根据各条工作内容分别判断。
...
2. 查询目标日期已有记录，并查询目标日期前连续 35 个自然日内当前填写人的全部分页日报，用于计算历史基线；从这批结果中取最近 20 条作为历史样本，用其中最近一条 `dailyType` 有效的日志确定本次类型，并统一用于全部补充记录，不根据工作内容猜测。若 35 日范围内不足 20 条，只使用已有记录，不扩大日期范围；同时按业务日期汇总目标日已有 `workHours`，仅把 `createTime` 视为历史填写时间，不能据此推断上下班时间。
